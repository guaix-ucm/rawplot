...
def add_args(parser):
    ...
    parser_raw.add_argument('-y', '--y0', type=vfloat01,  help='Normalized ROI start point, y0 coordinate [0..1]')
    parser_raw.add_argument('-wi', '--width',  type=vfloat01, default=1.0, help='Normalized ROI width [0..1] (default: %(default)s)')
    parser_raw.add_argument('-he', '--height', type=vfloat01, default=1.0, help='Normalized ROI height [0..1] (default: %(default)s) ')
    parser_raw.add_argument('-c','--channels', default=('R', 'Gr', 'Gb','B'), nargs='+',
                    choices=('R', 'Gr', 'Gb', 'G', 'B'),
    ...
    parser_corr.add_argument('-y', '--y0', type=vfloat01,  help='Normalized ROI start point, y0 coordinate [0..1]')
    parser_corr.add_argument('-wi', '--width',  type=vfloat01, default=1.0, help='Normalized ROI width [0..1] (default: %(default)s)')
    parser_corr.add_argument('-he', '--height', type=vfloat01, default=1.0, help='Normalized ROI height [0..1] (default: %(default)s) ')
    parser_corr.add_argument('-c','--channels', default=('R', 'Gr', 'Gb','B'), nargs='+',
                    choices=('R', 'Gr', 'Gb', 'G', 'B'),
                    help='color plane to plot. G is the average of G1 & G2. (default: %(default)s)')
    parser_corr.add_argument('--every', type=int, metavar='<N>', default=1, help='pick every n `file after sorting')
    parser_corr.add_argument('-bi', '--bias',  type=vflopath,  help='Bias, either a single value for all channels or else a 3D FITS cube file (default: %(default)s)')
    parser_corr.add_argument('-dk', '--dark',  type=vfloat,  help='Dark count rate in DN/sec. (default: %(default)s)')
    parser_corr.add_argument('-cv', '--csv-file', type=vfile, required=True, help='CSV file with photdiode readings')
    ...
    dioex1.add_argument('-n', '--normalized', action='store_true', help='Plot Photodiode normalized readings & QE')
    dioex1.add_argument('-s', '--signal-to-noise', action='store_true', help='Plot Raw Signal to Noise Ratio')
    parser_diode.add_argument('-m','--model', default=OSI_PHOTODIODE, choices=(HAMAMATSU_PHOTODIODE, OSI_PHOTODIODE),
                    help='Photodiode model. (default: %(default)s)')
    parser_diode.add_argument('-r','--resolution', type=int, default=5, choices=(1,5), 
                    help='Wavelength resolution (nm). (default: %(default)s nm)')
