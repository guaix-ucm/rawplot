...
def model_snr(signal, read_noise, p_fpn, gain = 1.0):
    '''All units in DN, vcan be used with e- if gain is 1.0'''
    return signal/(np.sqrt(read_noise**2 + (signal/gain) +(p_fpn*signal)**2))

def check_model(args):
    return True if args.read_noise is not None and args.p_fpn is not None and args.gain is not None else False
